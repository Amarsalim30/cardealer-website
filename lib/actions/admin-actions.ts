...
export async function bulkVehicleAction(
  ...
  const ids = [...new Set(formData.getAll("ids").map((id) => String(id).trim()).filter(Boolean))];

  if (!ids.length) {
    return actionFailure("Select at least one vehicle.") as BulkVehicleActionState;
  }

  if (!(["publish", "unpublish", "sold", "delete"] as const).includes(action)) {
    return actionFailure("Bulk action is invalid.") as BulkVehicleActionState;
  }
  ...
  for (const id of ids) {
    try {
      if (action === "delete") {
        ...
        const nextStatus = action === "publish" ? "published" : action === "unpublish" ? "unpublished" : "sold";
